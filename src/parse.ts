...
// Standardizes IES data that has a 90 degree horizontal sweep by repeating
// samples.
...
// Standardizes IES data that has a 180 degree horizontal sweep by repeating
// samples.
...
// documents that only sweep 90 degrees horizontal and others that only contain
// a single horizontal sweep. This function standardizes the passed IES document
// to include a complete 360 degree horizontal sweep.
function standardizeHorizontalAngles(data: IesData): void {
  const lastHorizontalAngle =
    data.photometricData.horizontalAngles[
      data.photometricData.horizontalAngles.length - 1
    ];
...
/**
 * Parses the given IES document into an IesData object. Throws an exception if
 * an error occurs.
 */
